<template>
    <div v-frag>
        <v-checkbox class="ml-2" v-model="newAutoSave" @change="onChangeAutoSave" hide-details dense label="AutoSave"></v-checkbox>
        <v-checkbox data-test="quick-edit" class="mr-2" :value="textMode" hide-details dense label="Quick Edit" @change="toggleTextMode"></v-checkbox>
        <v-spacer></v-spacer>
        <div class="d-flex align-center justify-end">
            <v-btn class="mx-2" color="primary" x-small @click="showSaveModal">Save in cache</v-btn>
            <v-btn class="mx-2" x-small @click="showCacheList">
                Cache
                <v-icon x-small right>fas fa-cog</v-icon>
            </v-btn>
            <v-btn class="mx-2" color="info" x-small @click="importFile">Import a Data</v-btn>
            <input
                    type="file"
                    v-show="false"
                    ref="import_file"
...
<!--            <v-btn class="mx-2" color="success" x-small @click="exportResume">Export</v-btn>-->
        </div>
    </div>
</template>
